...
public class DB {
    private static final String DATABASE_NAME = "points2.db";
    private static final int DATABASE_VERSION = 4;
    private static final String TABLE_NAME = "ForPoints1";
...
//    }
 ...
 public ArrayList<NewPoint> findPointsBySpecialParametr(String status){
     ArrayList<NewPoint> arrPoint = new ArrayList<>();
     ArrayList<NewPoint> arrPoint2 = new ArrayList<>();
//        SQLiteDatabase db = getReadableDatabase();
     ...
     if(cursor!=null && cursor.getCount()>0 && cursor.moveToFirst()){
         ...
         do {
             ...
             byte[] photo = cursor.getBlob(NUM_COLUMN_Image);
             ...
             NewPoint point  = new NewPoint(des ,photo, x, y, petName, petColor, petContact, petStatus);
             arrPoint2.add(new NewPoint(des ,photo, x, y, petName, petColor, petContact, petStatus));
             if(point.status.equals(status))
             {
                 arrPoint.add(point);
             }
         ...
         if (status.equals("Все") || status.equals(""))
         {
             return arrPoint2;
         }
         else{
    ...
    public ArrayList<NewPoint> findAllPoints(){
        ArrayList<NewPoint> arrPoint = new ArrayList<>();
//        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = MyDataBase.query(TABLE_NAME,   null, null, null, null, null, null);
        if(cursor!=null && cursor.getCount()>0 && cursor.moveToFirst()){
            ...
            do {
                ...
                arrPoint.add(new NewPoint(des ,photo, x, y, petName, petColor, petContact, petStatus));
            }while (cursor.moveToNext());
            return arrPoint;
        }
        else{
